            ...
            try
            {
                ...
                using (TextReader reader = new StreamReader(file))
                {
                    ...
                    while ((line = reader.ReadLine()) != null)
                    {
                        ...
                        if (items.Length < 5)
                        {
                            Console.WriteLine("Line {0} doesn't contain occurrences ({1})", i, items.Length > 0 ? items[0] : "(no subject)");
                            continue;
                        }
